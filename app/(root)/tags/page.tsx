...
const Page = async ({ searchParams }: SearchParamsProps) => {
  ...
  return (
    <>
      <h1 className='h1-bold text-dark100_light900'>所有标签</h1>
      ...
      <section className='mt-12 flex flex-wrap gap-4'>
        {result.tags.length > 0 ? (
            ...
            <Link href={`/tags/${tag._id}`} key={tag._id}>
              <article
                ...
                <div
                  ...
                  <p
                    className='paragraph-semibold text-dark300_light900'>{tag.name}</p>
                </div>

                <p className='small-medium text-dark400_light500 mt-3.5'>
                  <span
                    className='body-semibold primary-text-gradient mr-2.5'>{tag.questions.length}+</span> 问题
                </p>
              </article>
            </Link>
          ))
          ...
          <NoResult
            title='暂无标签'
            description='如果你愿意的话，你可以发起一根问题，并且在【标签】中新增一个标签'
            link='/ask-question'
            linkTitle='发起问题'
          />
        )}
